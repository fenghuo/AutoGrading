...
int main(int argc, char *argv[]){
	...
	goto my_label2;
	my_label1:
	m[intPow(75, 1)] = intPow(5, 1) - intPow((intPow(m[intPow(2, 1)], 1) - intPow(6, intPow(2, 1))), 1);
	m[intPow(13, 1)] = intPow(m[intPow(2, 1)], 1) * intPow(m[intPow(0, 1)], 1);
	my_label2:
	m[intPow(75, 1)] = intPow(5, 1) - intPow((intPow(m[intPow(0, 1)], 1) - intPow(6, 1)), 1);
	m[intPow(13, 1)] = intPow(m[intPow(2, 1)], 1) * intPow(m[intPow(0, 1)], 1);
	goto my_label3;
	printf("%d\n", intPow(5, 1) * intPow(5, 1));
	my_label3:
	m[intPow(10, 1)] = intPow(1334, 1);
	printf("%d\n", intPow(m[intPow(0, 1)], 1));
	printf("%d\n", intPow(m[intPow(2, 1)], 1));
	printf("%d\n", intPow(m[intPow(13, 1)], 1));
	printf("%d\n", intPow(m[intPow(10, 1)], 1));
