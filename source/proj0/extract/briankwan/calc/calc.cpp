...
token_type scanner_t::next_token()
{
	...
	char single;
	character = cin.peek();
	if(character == ' ') {cin.get(character); character = cin.peek();}
	if(character == '\n') {
	...
	while(num <10 && num >= 0){
		ss << num;
		//cerr<<"STREAM "+ss.str()<<endl;
		global = global + ss.str();
		//cout << together;
		cin.get(character);
		num = cin.peek() - '0';
		a = 1;
	...
	if(a == 1) { cin.putback(character); return T_num;}
	...
	switch(character){
		...
		case '*':  
			cin.get(character);
			if(cin.peek() == '*') { cin.putback(character);return T_power;}
			else { cin.putback(character); return T_times;}
...
void scanner_t::eat_token(token_type c)
{
	...
	if(next_token() == c){
		...
		if(c == T_power){cin.get(character);}
		if(c == T_goto){
			if(cin.peek() == 'g'){
			cin.get(character);}
			if(cin.peek() == 'o') {
		...
		if(c == T_if){
			if(cin.peek() == 'i'){
			cin.get(character);}
			if(cin.peek() == 'f') {cin.get(character);}
