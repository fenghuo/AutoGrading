...
char* nonterm_to_string(nonterm_type nt)
{
	...
	switch( nt ) {
		  ...
		  case NT_Expr_1: strncpy(buffer,"Expression_1",MAX_SYMBOL_NAME_SIZE); break;
		  case NT_Term_1: strncpy(buffer,"Term_1",MAX_SYMBOL_NAME_SIZE); break;
		  case NT_Term_2: strncpy(buffer,"Term_2",MAX_SYMBOL_NAME_SIZE); break;
		  case NT_Term_3: strncpy(buffer,"Term_3",MAX_SYMBOL_NAME_SIZE); break;
		  ...
		  case NT_Pt: strncpy(buffer,"Pt",MAX_SYMBOL_NAME_SIZE); break;
...
token_type scanner_t::next_token()
{
	...
	if(temp_token_exist) return temp_token;
	int t = cin.get();
	switch(t) {
		case '+': temp_token = T_plus; break;
		case '-': temp_token = T_minus; break;
		case '/': temp_token = T_divide; break;
		case '*': {
			if(cin.get() == '*') {
				temp_token = T_power; break;
			}
			else {
				cin.unget();
				temp_token = T_times; break;
			}
		}
		case '=': temp_token = T_equals; break;
		case '(': temp_token = T_openparen; break;
		case ')': temp_token = T_closeparen; break;
		case '[': temp_token = T_opensquare; break;
		case ']': temp_token = T_closesquare; break;
		case 'm': temp_token = T_m; break;
		case '\n': line_num=line_num+1; return next_token(); break;
		...
		case 'g': {
			if(cin.get() == 'o'){}
				...
				if(cin.get() == 't'){}
					...
					if(cin.get() == 'o'){
						temp_token = T_goto; break;
					}
					else cin.unget();
		}
		case 'i': {
			if(cin.get() == 'f'){
				temp_token = T_if; break;
			}
			else cin.unget();
		}
		case EOF: temp_token = T_eof; break;
		case 'p': {
			if(cin.get() == 'r'){}
				else cin.unget();
				if(cin.get() == 'i'){}
					...
					if(cin.get() == 'n'){}
						...
						if(cin.get() == 't'){
							temp_token = T_print; break;
							}
						else cin.unget();
		}
		case 'L': temp_token = T_label; break;
		case ':': temp_token = T_colon; break;
		default:
			long num_temp = t - '0';
			long num_tens = 10;
			if (t >= '0' && t<= '9') {
				...
				while(true) {
					int t = cin.get();
					if(!(t >= '0' && t<= '9')) break;
					num_temp = num_temp + num_tens * (t - '0');
				}
				cin.unget();
			}else {
				scan_error(t);
...
//Here is an example
void parser_t::Start_pre(){
	string c_code_header = "#include <stdio.h>; \n#include <stdlib.h>; \n#include <assert.h>; \n#include <string.h>;\n#include <ctype.h>; \n#include <stack>; \n#include <iostream>; \nusing namespace std; \nint m[101];";
	string c_code = "int main() { ";
	Start();
	c_code = c_code + "return 0;}";
	fprintf(stderr, "%s\n%s", c_code_header.c_str(), c_code.c_str());
}
void parser_t::Start()
{
...
void parser_t::Expr()
{
	...
	switch( scanner.next_token() ) 
	{
		...
		case T_label:
		case T_num:
		case T_openparen:  //added
		case T_opensquare: //added
			Term_1();
			Expr_1();
			break;
		default:
...
void parser_t::Expr_1()
{
	//push this non-terminal onto the parse tree.
	//the parsetree class is just for drawing the finished
	//parse tree, and should in should have no effect the actual
	//parsing of the data
	parsetree.push(NT_Expr_1);
	...
	switch( scanner.next_token() ) 
	{
		...
		case T_minus:
			...
			Expr();
			break;
		case T_closesquare: //substracked open of each [  and (
		case T_closeparen:
		case T_equals:
		case T_num:
		case T_m:
...
void parser_t::Term_1()
{
	//push this non-terminal onto the parse tree.
	//the parsetree class is just for drawing the finished
	//parse tree, and should in should have no effect the actual
	//parsing of the data
	parsetree.push(NT_Term_1);
...
void parser_t::Pt()
{
	parsetree.push(NT_Pt);
	Factor();
	Term_3();
	parsetree.pop();

}
void parser_t::Term_2()
{
	//push this non-terminal onto the parse tree.
	//the parsetree class is just for drawing the finished
	//parse tree, and should in should have no effect the actual
	//parsing of the data
	parsetree.push(NT_Term_2);
...
void parser_t::Term_3()
{
	//push this non-terminal onto the parse tree.
	//the parsetree class is just for drawing the finished
	//parse tree, and should in should have no effect the actual
	//parsing of the data
	parsetree.push(NT_Term_3);
...
void parser_t::Factor()
{
	...
	switch( scanner.next_token() ) 
	{
		case T_m:
			...
			break;
		case T_goto:
			eat_token(T_goto); break;
		case T_label:
			eat_token(T_label); break;
		case T_openparen:
...
// in parsr star function make a c code pre = with all the includes 
//c code main = int main() { ";
//the you call the first switch statement at the end
//c code main = c code main + "return 0;}"
//fprintf(stderr, %s\n%s, c code pre.c_str(), c_code_main.c_str());
//print out terminals in c
