...
char* nonterm_to_string(nonterm_type nt)
{
  ...
  switch( nt ) {
	...
	case NT_Expression: strncpy(buffer,"Expression",MAX_SYMBOL_NAME_SIZE); break;
        ...
        case NT_TPrime: strncpy(buffer,"TPrime",MAX_SYMBOL_NAME_SIZE); break;
...
token_type scanner_t::next_token()
{
  ...
  token_type nextToken = tokens.front();
  while (nextToken == T_endline) {
    line++;
    eat_token(T_endline);
...
scanner_t::scanner_t()
{
  ...
  //read file and fill queue (include tokens for line breaks)
}
