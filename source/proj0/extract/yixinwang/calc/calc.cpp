...
char* nonterm_to_string(nonterm_type nt)
{
	...
	case NT_Derivestate: strncpy(buffer, "Derivestate",MAX_SYMBOL_NAME_SIZE);break;
	case NT_Secondjump: strncpy(buffer, "Secondjump",MAX_SYMBOL_NAME_SIZE);break;
	...
	case NT_Q: strncpy(buffer,"Q",MAX_SYMBOL_NAME_SIZE);break;
...
int main()
{
  ...
  cerr<<" m[i]=0;}\n";
	...
	//scanner_t Scan;
  //  for(int i=0;i<200;i++)
  //while(Scan.next_token()!=T_eof)	
  //{
  //	  cout<<token_to_string(Scan.next_token());
  
  // 	Scan.eat_token(Scan.next_token());
  //	  	 }
		return 0;
	}
