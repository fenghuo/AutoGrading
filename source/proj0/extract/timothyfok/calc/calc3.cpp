...
token_type scanner_t::next_token()
{
  ...
  if (x == 1)
    ...
    if (ch == '+'){mismatch_token = T_plus;  return T_plus; }
    if (ch == '-'){mismatch_token = T_minus;  return T_minus; }
    if (ch == '*'){mismatch_token = T_times;  return T_times; }
    if (ch == '/'){mismatch_token = T_divide;  return T_divide; }   
    if (ch == '('){mismatch_token = T_openparen;  return T_openparen; }
    if (ch == ')'){mismatch_token = T_closeparen;  return T_closeparen; }
    if (ch == '.'){scan_error(ch); exit(1);}
...
void scanner_t::eat_token(token_type c)
{
  ...
  while (ch == '0' || ch == '1' ||
	...
	decimalcount++;
	if (decimalcount > 1)
	  {scan_error(ch); exit(1);
	  }
      }
    ch = getchar();
  }
  //  cout << "THE NUMBER STREAM IS NOW " << numberstream << "!!" << endl;

  numbercheck = atof(numberstream.c_str());
  if (numbercheck < 0 || (numbercheck > pow(2,31) - 1)){
    scan_error(ch); exit(1);
    //cout << "WHAT IS THIS!!!";
  }
  //  cout << "THE NUMBER CHECK IS NOW " << numbercheck << "!!!" << endl;
