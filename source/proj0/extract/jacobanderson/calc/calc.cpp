...
char* nonterm_to_string(nonterm_type nt)
{
	...
	switch( nt ) {
		...
		case NT_Stmt: strncpy(buffer,"Statement",MAX_SYMBOL_NAME_SIZE); break;
		...
		case NT_Power_prime: strncpy(buffer,"Power_prime",MAX_SYMBOL_NAME_SIZE); break;
...
class scanner_t {
  ...
  private:
	...
	//characters one at a time (using cin.gethar would be a good way)
	...
	//error message if T_num is out of range 0-2^31-1 inclusive
	void range_error(int x);
...
token_type scanner_t::next_token()
{
	...
	switch(cin.peek())
	{
		...
		case 'm': return T_m;
		case 'g': return T_goto;
		case 'i': return T_if;
		case 'p': return T_print;
		case 'L': return T_label;
		case ':': return T_colon;
		case ' ': cin.get(); return next_token(); break;
		case '\n': line++; cin.get(); return next_token(); break;
		default: 
...
void scanner_t::eat_token(token_type c)
{
	...
	switch(c){
		...
		case T_num:
			cin >> i;
			if (i > 2147483647) range_error(i);
			cerr << i;
			break;
		case T_plus: cin.get(); break;
		case T_minus: cin.get(); break;
		case T_times: cin.get(); break;
		case T_divide: cin.get(); break;
		case T_power: cin.get(); cin.get(); break;
		case T_equals: cin.get(); break;
		case T_openparen: cin.get(); break;
		case T_closeparen: cin.get(); break;
		case T_opensquare: cin.get(); break;
		case T_closesquare: cin.get(); break;
		case T_m: cin.get(); break;
		case T_goto:
			if ((x=cin.get())!='g' || (x=cin.get())!='o'
			||(x=cin.get())!='t' || (x=cin.get())!='o')
				scan_error(x);
			break;
		case T_if:
			if ((x=cin.get())!='i' || (x=cin.get())!='f')
				scan_error(x);
			break;
		case T_print:
			if ((x=cin.get())!='p' || (x=cin.get())!='r'
			||(x=cin.get())!='i' || (x=cin.get())!='n' || (x=cin.get())!='t')
				scan_error(x);
			break;
		case T_label: cin.get(); break;
		case T_colon: cin.get(); break;
	}
}
...
void scanner_t::range_error (int x)
{
	printf("range error: int '%i' out of range -line %d\n",x, get_line());
	exit(1);
}
...
void parser_t::eat_token(token_type t)
{
	...
	scanner.eat_token(t);
	switch(t){
		case T_print: cerr << "printf(\"%i\","; break;
		case T_power: case T_num: break;
		default:
			cerr << token_to_string(t);
			break;
	}
	parsetree.pop();
}
...
void parser_t::Jump(){
	...
	stringstream buffer;
	streambuf *backup = cerr.rdbuf(buffer.rdbuf());
	eat_token(T_goto);
	cerr << " L";
	eat_token(T_num);
	cerr.rdbuf(backup);
...
void parser_t::Power(){
	...
	stringstream pow_lhs, pow_rhs;
	streambuf *backup;
	switch(scanner.next_token()){
		case T_num:
		case T_openparen:
		case T_m:
			backup=cerr.rdbuf(pow_lhs.rdbuf());
			Factor();
			cerr.rdbuf(backup);
			backup=cerr.rdbuf(pow_rhs.rdbuf());
			Power_prime();
			cerr.rdbuf(backup);
			cerr << ((pow_rhs.str()=="") ? pow_lhs.str() : pow_rhs.str().replace(2,3,pow_lhs.str()));
			break;
		default:
...
void parser_t::Power_prime(){
	parsetree.push(NT_Power_prime);
	stringstream pow_lhs, pow_rhs;
	streambuf *backup;
	switch(scanner.next_token()){
		case T_power:
			...
			Power_prime();
			cerr.rdbuf(backup);
			cerr << "e(LHS," << (pow_rhs.str()=="" ? pow_lhs.str() : pow_rhs.str().replace(2,3,pow_lhs.str())) << ")";
			break;
		case T_eof:
		case T_label:
		case T_goto:
